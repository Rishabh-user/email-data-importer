...
def process_file(raw_file: RawFile):
    ...
    try:
        ...
        raw_json = make_json_safe(extracted_output.get("raw_json"))
        ...
        for idx, row in enumerate(extracted_rows, start=1):
            try:
                ...
                extracted = ExtractedRecord.objects.create(
                    raw_file=raw_file,
                    po_number=row.get("PO") or row.get("PO Number"),
                    customer_part=row.get("ERP Code") or row.get("Customer Material Number"),
                    description=row.get("Description"),
                    quantity=row.get("Qty Ordered"),
                    open_qty=row.get("Open Sched Qty") or row.get("Balance Due"),
                    need_date=parse_date(str(row.get("Need Date"))) if row.get("Need Date") else None,
                    promised_date=parse_date(str(row.get("Promised Date"))) if row.get("Promised Date") else None,
                    ship_date=parse_date(str(row.get("Ship Date"))) if row.get("Ship Date") else None,
                    full_row_json=row,
                )
